...
const OAuthButtons: React.FC<OAuthButtonsProps> = ({
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'facebook') => {
    try {
      ...
      // Note: On success, user will be redirected to callback page
  ...
  const isButtonLoading = (provider: string) => {
    return isLoading || localLoading === provider
  }
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      >
        ...
        {isButtonLoading('google') ? 'Signing in...' : 'Continue with Google'}
      </Button>
      ...
      >
        ...
        {isButtonLoading('facebook') ? 'Signing in...' : 'Continue with Facebook'}
      </Button>
