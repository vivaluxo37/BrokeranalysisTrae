...
>(({ className, children, hideTitle = false, hideDescription = false, ...props }, ref) => {
  // More robust check for DialogTitle and DialogDescription
  const hasTitle = React.useMemo(() => {
    ...
    return childrenArray.some((child) => {
      if (React.isValidElement(child)) {
        ...
        // Check for DialogTitle in nested components (like DialogHeader)
        if (child.props?.children) {
          const nestedChildren = React.Children.toArray(child.props.children)
          return nestedChildren.some((nestedChild) => 
            React.isValidElement(nestedChild) && nestedChild.type === DialogTitle
          )
        }
      }
      return false
  ...
  const hasDescription = React.useMemo(() => {
    ...
    return childrenArray.some((child) => {
      if (React.isValidElement(child)) {
        ...
        // Check for DialogDescription in nested components
        if (child.props?.children) {
          const nestedChildren = React.Children.toArray(child.props.children)
          return nestedChildren.some((nestedChild) => 
            React.isValidElement(nestedChild) && nestedChild.type === DialogDescription
          )
        }
      }
      return false
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        {...props}
      >
        {/* Always add hidden title and description for accessibility */}
